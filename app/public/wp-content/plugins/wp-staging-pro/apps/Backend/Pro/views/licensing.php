<div class="wpstg_admin">
    ...
    <span class="wpstg-version">
        <?php echo 'Pro v.' . \WPStaging\WPStaging::VERSION ?>
    </span>
...
<div>
    <label for="wpstg_license_key" style='display:block;margin-bottom: 5px;margin-top:10px;'><?php _e('Enter License Key to activate WP Staging Pro:','wpstg'); ?></label>
      ...
      if (isset($license->error) && 'expired' === $license->error){
         $message =  '<span style="color:red;">' . __('Your license expired on ', 'wpstg') . date_i18n( get_option( 'date_format' ), strtotime( $license->expires, current_time( 'timestamp' ) )) . '</span>';
      } else if (isset($license->license) && 'valid' === $license->license) {
         $message =  __('Your license is valid until ', 'wpstg') . date_i18n( get_option( 'date_format' ), strtotime( $license->expires, current_time( 'timestamp' ) ));
      ...
      if( isset( $license->license ) && 'valid' === $license->license ) {
         ...
         echo '<input type="submit" class="button" value="' . __( 'Deactivate License', 'wpstg' ) . '">';
      } else {
         echo '<input type="hidden" name="wpstg_activate_license" value="1" />';
         echo '<input type="submit" class="button-primary" value="' . __( 'Activate License', 'wpstg' ) . '">';
      }
      ?>
