...
class SearchReplace extends \WPStaging\Backend\Modules\Jobs\JobExecutable {
    ...
    /**
     * Number of rows to use the search and replace method on
     * @var int
     */
    ...
    private function updateTable($tableName) {
       $strings = new Strings();
        //$table = str_replace($this->options->prefix, '', $tableName);
    ...
    private function startReplace($new) {
        ...
        $this->log(
                "DB Search & Replace:  {$this->options->job->start} to {$rows} records"
        );
    ...
    private function searchReplace($table, $page, $args) {
      ...
      //$args['search_for'] = $this->options->existingClones[$this->options->current]['url'];
      //$args['replace_with'] = rtrim(get_home_url(), '/');
      // Search URL example.com/staging and root path to staging site /var/www/htdocs/staging
      $args['search_for'] = array(
          $this->options->existingClones[strtolower($this->options->current)]['url'],
          ABSPATH . $this->options->existingClones[strtolower($this->options->current)]['directoryName']
      );
...
//                           //$edited_data = $this->recursive_unserialize_replace($args['search_for'][$i], $replace['replace_with'][$i], $dataRow, false, $args['case_insensitive']);
//                           $dataRow = $this->recursive_unserialize_replace($args['search_for'][$i], $args['replace_with'][$i], $dataRow, false, $args['case_insensitive']);                           
//                           //$this->log('DB Search & Replace: '.$table.' - Replace ' . $args['search_for'][$i] . ' with ' . $args['replace_with'][$i]);
//                           $i++;
//                        }
//                        //unset($replace);
//                        //unset($i);
//                        //if ($edited_data != $dataRow) {
...
//                    }
//
//                    if ('siteurl' === $dataRow && $args['dry_run'] !== 'on') {
//                        $update_later = true;
//                    }
//                }
               ...
               $i = 0;
               foreach ($args['search_for'] as $replace){
                  //$edited_data = $this->recursive_unserialize_replace($args['search_for'][$i], $args['replace_with'][$i], $dataRow, false, $args['case_insensitive']);
                  $dataRow = $this->recursive_unserialize_replace($args['search_for'][$i], $args['replace_with'][$i], $dataRow, false, $args['case_insensitive']);
            ...
            if ($args['dry_run'] === 'on') {
                ...
                if (!$result) {
                    //$table_report['errors'][] = sprintf(__('Error updating row: %d.', 'wpstg'), $current_row);
                } else {
