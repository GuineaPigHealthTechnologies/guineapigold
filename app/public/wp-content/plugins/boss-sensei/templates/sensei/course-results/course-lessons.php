...
if ( is_user_logged_in() ) {
	...
	$html .= '<section class="course-results-lessons">';
        ...
        foreach( $modules as $module ) {
            ...
            if( count( $lessons ) > 0 ) {
                ...
                foreach( $lessons as $lesson_item ) {
                    ...
                    $html .= '<h2><a href="' . esc_url( get_permalink( $lesson_item->ID ) ) . '" title="' . esc_attr( sprintf( __( 'Start %s', 'boss-sensei' ), $lesson_item->post_title ) ) . '">' . esc_html( sprintf( __( '%s', 'boss-sensei' ), $lesson_item->post_title ) ) . '</a> <span class="lesson-grade">' . $lesson_grade . '</span></h2>';
		...
		if(  0 < count( $displayed_lessons ) ) {
			$html .= '<h3>' . __( 'Lessons', 'boss-sensei' ) . '</h3>' . "\n";
		}
		...
		$html .= '<h2 class="total-grade">' . apply_filters( 'sensei_total_grade_text', __( 'Total Grade', 'boss-sensei' ) ) . '<span class="lesson-grade">' . $course_user_grade . '%</span></h2>';
