...
global $class, $groups_template, $post;
	...
	while ( bp_groups() ) {
		...
		<div class="page-right-sidebar <?php echo $class . $wrap_class; ?>">
			...
			if ( $group_status ) {
				...
				$course_class = '';
				if ( !bp_is_current_action( 'invite-anyone' ) ) {
					$course_class = 'course';
				}

				echo '<article class="' . $course_class . ' post type-course">';
			}
			?>
			...
			<div id="buddypress">
				...
				<?php if ( $group_status ) { ?>
					...
					<section class="course-header">
						<div class="table top">
							<?php
							$img = get_the_post_thumbnail( $post->ID, 'course-single-thumb', array( 'class' => 'woo-image thumbnail alignleft' ) );
					...
					<section id="course-details">
						<span class="course-statistic">
                            ...
                            if ( function_exists('Sensei_Course_Participants') ) {
                                ...
                                $learner_count = WooThemes_Sensei_Utils::sensei_check_for_activity( $activity_args, false );

                                echo '<p class="sensei-course-participants">' . sprintf( __( '%s %s taking this course', 'boss-sensei' ), '<strong>' . intval( $learner_count ) . '</strong>', _n( 'learner', 'learners', intval( $learner_count ), 'boss-sensei' ) ) . '</p>' . "\n";
                            }
                            ?>
						</span>
						<div class="course-buttons">
							 ...
							 */
							setup_postdata($post);
							echo Sensei_Course::the_course_enrolment_actions();
							wp_reset_postdata();
							?>
						</div>
					</section>
				...
				if ( $group_status == '' ) {
					echo '<div id="primary" class="site-content">';
					echo '<div id="content" role="main">';
					echo '<div class="below-cover-photo">';
					echo '<div id="group-description">';
		...
		</div><!-- `.page-right-sidebar` closing div -->
		<?php
	}
}