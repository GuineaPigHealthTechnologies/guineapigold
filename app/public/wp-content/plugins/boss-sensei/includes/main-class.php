...
class Boss_Sensei_Plugin
{
    ...
    private function setup_actions() {
        ...
        if($woothemes_sensei){
            ...
            remove_action( 'sensei_single_course_content_inside_before', array( $get_sensei_class_object, 'send_message_link' ), 35 );
            ...
            //add_filter( 'sensei_start_course_text', array( $this, 'boss_edu_filter_start_button' ) ); This is deprecated since 1.9.0
            ...
            if ( function_exists('Sensei_Course_Participants') ) {
                remove_action( 'sensei_single_course_content_inside_before', array( Sensei_Course_Participants(), 'display_course_participant_count' ), 15 );
            ...
            // My courses tabs courses count ( Not working since 1.9.0, filters removed )
            ...
            // Load course results
            remove_action( 'sensei_course_results_content_inside_before_lessons', array( $woothemes_sensei->course_results, 'course_info' ), 10 );
            add_action( 'sensei_course_results_content_inside_before_lessons', array( $this, 'boss_edu_course_info' ), 10 );
            ...
            if( $woothemes_sensei->version < '1.9.0' ){
                ...
                add_filter( 'template_include', array ( $this, 'template_loader' ), 10, 2 );
            }
        }
        ...
        add_filter( 'author_template', array($this, 'boss_edu_archive_template') );
        
        add_action( 'wp_head', array( $this, 'boss_edu_course_group_canonical_link' ), 10 );

        add_filter( 'body_class', array( $this, 'boss_edu_my_courses_class' ) );
    ...
    // Change author template
    function boss_edu_archive_template( $template ) {
        // Adjust with your custom post types.
        if ( is_archive() ) {
            if ( file_exists( $this->templates_dir . '/sensei-buddypress/author.php' ) ) {
                $template = $this->templates_dir . '/sensei-buddypress/author.php';
            }
        }
        return $template;
    }
...
//                $output .= '</div><!-- .badgeos-item-excerpt -->';
    ...
    */
    public function boss_edu_overide_sensei_courses_template() {
        global $woothemes_sensei, $post;
         ...
         $course_page       = $woothemes_sensei->settings->get( 'course_page' ); //Course archive page
         $my_course_page    = $woothemes_sensei->settings->get( 'my_course_page' ); //My Course Page

         if (  ! empty( $post->ID ) && ( $post->ID == $course_page || $post->ID == $my_course_page ) ) {
            //Include Sensei Courses Sidebar template
    ...
    */
    function boss_edu_overide_courses_html() {
        if ( function_exists('bp_is_active') && function_exists( 'buddypress_sensei' ) && bp_is_active('groups')) {
    ...
    */
	public function boss_edu_filter_course_activity( $action ) {
			...
			global $activities_template,$wpdb;
            ...
            $action_text = $wpdb->get_var( $wpdb->prepare("SELECT action FROM {$wpdb->prefix}bp_activity WHERE id = %d", $act_id ) );
            ...
            if ( $is_course_act ) {
                ...
                if ( strpos( $action_text, 'started taking the course' ) != false ) {
                ...
                } elseif( strpos( $action_text, 'has passed the' ) != false ) {
                    ...
                    $html .= '</div>';
                    $action .= $html;
                } elseif( strpos( $action_text, 'completed the lesson' ) != false ) {
                    $html = '<div class="bp-sensei-activity table course-activity">';
                        $html .= '<div class="table-cell edu-activity-image">';
    ...
    public function boss_edu_sensei_active_courses_text(){
        global $woothemes_sensei, $learner_user;
        ...
        foreach( $course_statuses as $course_status ) {
            if ( WooThemes_Sensei_Utils::user_completed_course( $course_status, $learner_user->ID ) ) {
    ...
    */
    ...
    public function boss_edu_get_group_template() {
        $this->boss_edu_load_template('/sensei-buddypress/buddypress-group-single' );
    }
     ...
     * @return void
     */
    public function boss_edu_course_module_content( $course_id ){
        ...
        foreach ($modules as $module) {
            ...
            echo '<header class="expList"><h2 class="expList"><a href="' . esc_url($module_url) . '">' . $module->name . '</a><div class="feat-right">';
            ...
            if (count($lessons) > 0) {
                echo '<i class="expList fa tog-mod fa-chevron-down"></i>';
            }
...
//                            } // End If Statement
                        ...
                        $meter_class = ($progress_percentage)?'':'zero'; 
                        $active_html .= '<div class="meter-bottom '.$meter_class.'"><span style="width: ' . $progress_percentage . '%"><span>' . $progress_percentage . '%</span></span></div>';
			...
			if( $active_count > $per_page ) {
				...
				$active_html .= '<div class="pagination woo-pagination">';
				$total_pages = ceil( $active_count / $per_page );

                $active_html .= paginate_links( array(
                    'base'         => add_query_arg('active_page','%#%'),
                    'format'       => '?active_page=%#%',
                    'add_args'     => false,
...
//		    		    	} // End If Statement
			...
			if( $completed_count > $per_page ) {
				...
				$complete_html .= '<div class="pagination woo-pagination">';
				$total_pages = ceil( $completed_count / $per_page );

                $complete_html .= paginate_links( array(
                    'base'         => add_query_arg('completed_page','%#%'),
                    'format'       => '?completed_page=%#%',
                    'add_args'     => false,
                    'current'      => max( 1, $current_page ),
                    'total'        => $total_pages,
                    'prev_text'    => '&larr;',
    ...
    public function boss_edu_change_modules() {
        global $woothemes_sensei, $sensei_module_collapse;

        remove_action( 'sensei_single_course_content_inside_after', array( $woothemes_sensei->modules, 'load_course_module_content_template' ), 8 );
        add_action( 'sensei_single_course_content_inside_after', array( $this, 'boss_edu_course_module_content' ), 20);

        // Remove pango Sensei Module Collapse since it is added in plugin
        if ( class_exists( 'Sensei_Module_Collapse' ) ) {

            // Remove collapsible module content display Sensei < V1.9
            remove_action('sensei_single_course_modules_content', array( $sensei_module_collapse, 'course_module_collapse_content'), 20);

            // Remove collapsible module content display Sensei >= V1.9
            remove_action('sensei_single_course_content_inside_after', array( $sensei_module_collapse, 'load_course_module_collapse_content_template'), 8);

            // Remove collapsible module title for all Sensei versions
            remove_action('sensei_single_course_modules_before', array( $sensei_module_collapse, 'course_modules_collapse_title'), 21);
         }
    ...
    public function boss_edu_remove_bp_sensei_hooks() {
        ...
        if ( function_exists( 'buddypress_sensei' ) ) {
            remove_action('sensei_single_course_content_inside_before', array( buddypress_sensei()->bp_sensei_loader,'bp_sensei_send_message_link' ),110) ; 
            remove_action('sensei_single_course_content_inside_before', array( buddypress_sensei()->bp_sensei_groups,'bp_sensei_group_discussion_button' ),110);
        } 
    ...
    */
	...
	public function boss_edu_module_table_title() {
        ...
        $count_string = sprintf( _n( '%s Lesson', '%s Lessons', $count, 'boss-sensei' ), $count );
    ...
    */   
    public function boss_edu_wrapper_start() {
        ...
        if ( ( is_active_sidebar('sensei-lesson') && ( is_singular('lesson') || is_tax($woothemes_sensei->modules->taxonomy ) ) )
            ...
            || ( is_active_sidebar('sensei-courses') ) ):
    ...
    */ 
    public function boss_edu_wrapper_end() {
        ...
        elseif ( is_active_sidebar('sensei-courses') ) { $this->boss_edu_load_template('sidebar-sensei-courses'); }
    ...
    public function boss_edu_enqueue_scripts() 
    {
        ...
        if($woothemes_sensei) {
            ...
            //@todo need to remove badgeos style after pushing update for boss theme
            if(is_rtl()) {
            // Styles
//          wp_enqueue_style( 'sensei', $this->assets_url . '/css/sensei-rtl.css', array(), '1.0.8.1', 'all' );
            wp_enqueue_style( 'sensei', $this->assets_url . '/css/sensei-rtl.min.css', array(), BOSS_SENSEI_PLUGIN_VERSION, 'all' );
            wp_enqueue_style( 'sensei-badgeos', $this->assets_url . '/css/badgeos/badgeos-rtl.css', array(), BOSS_SENSEI_PLUGIN_VERSION, 'all' );
            } else {
            // Styles
//          wp_enqueue_style( 'sensei', $this->assets_url . '/css/sensei.css', array(), '1.0.8.1', 'all' );
            wp_enqueue_style( 'sensei', $this->assets_url . '/css/sensei.min.css', array(), BOSS_SENSEI_PLUGIN_VERSION, 'all' );
            wp_enqueue_style( 'sensei-badgeos', $this->assets_url . '/css/badgeos/badgeos.css', array(), BOSS_SENSEI_PLUGIN_VERSION, 'all' );
            }
            ...
            if($theme_layout == 'boxed') {
                if(is_rtl()){
                    wp_enqueue_style( 'boss-sensei-boxed', $this->assets_url . '/css/sensei-boxed-rtl.css', array(), BOSS_SENSEI_PLUGIN_VERSION, 'all' );
                } else {
                    wp_enqueue_style( 'boss-sensei-boxed', $this->assets_url . '/css/sensei-boxed.css', array(), BOSS_SENSEI_PLUGIN_VERSION, 'all' );
                }            
            }   
            
            // Javascript
//            wp_enqueue_script( 'sensei', $this->assets_url . '/js/sensei.js', array('jquery'), '1.0.7', true );
            wp_enqueue_script( 'sensei', $this->assets_url . '/js/sensei.min.js', array('jquery'), BOSS_SENSEI_PLUGIN_VERSION, true );
        }
    }
	 ...
	 *
	 */
    public function boss_edu_load_template( $template, $load = true ) {
        $template .= '.php';
        ...
        if( file_exists( STYLESHEETPATH.'/boss-sensei/'.$template ) )
            $located = STYLESHEETPATH.'/boss-sensei/'.$template;
        else if( file_exists( TEMPLATEPATH.'boss-sensei/'.$template ) )
            $located = TEMPLATEPATH.'/boss-sensei/'.$template;
        else{
            $template_dir = apply_filters( 'boss_edu_templates_dir_filter', $this->templates_dir );
            $located = trailingslashit( $template_dir ) . $template;
        }
    ...
    public function template_loader ( $template = '' ) {
        ...
        if ( $file ) {
            $template = $this->boss_edu_load_template("sensei/{$file}", false );
        }
    ...
    /**
     * Body class my-course-page
     * @param $classes
     * @return array
     */
    public function boss_edu_my_courses_class( $classes ) {
        global $post;
        if ( isset( $post ) && is_page() && Sensei()->settings->get( 'my_course_page' ) == $post->ID ) {
            $classes[] = 'my-course-page';
        }
        return $classes;
    }
