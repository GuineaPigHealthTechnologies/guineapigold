...
<?php
	...
	protected function load_component ( $instance ) {
        ...
        if ( function_exists('bp_is_active') ) {
            $author_avatar = bp_core_fetch_avatar ( array( 'item_id' => $teacher_id, 'type' => 'full', 'width' => '70', 'height' => '70' ) );
            $author_url = bp_core_get_user_domain( $post->post_author );
            $author = '<a href="' . $author_url . '">' . bp_core_get_user_displayname( $teacher_id ) . '</a>';
        } else {
            $author_avatar = get_avatar( $teacher_id, 70 );
            $author = get_the_author_meta( 'display_name', $teacher_id );
        }
        ?>
                ...
                if( ! isset( $woothemes_sensei->settings->settings['messages_disable'] ) || ! $woothemes_sensei->settings->settings['messages_disable'] ) {
                    ...
                    if( is_user_logged_in() ) {
                        ...
                        if( isset( $_GET['contact'] ) ) {
                            ...
                            if( bp_is_active( 'messages' ) ) {
                                $compose_link = wp_nonce_url( bp_loggedin_user_domain() . bp_get_messages_slug() . '/compose/?r=' . bp_core_get_username( $teacher_id ) );
                                $html .= '<p><a class="button send-message-button" href="' . esc_url($compose_link) . '#'.$id.'">' . $contact_button_text . '</a></p>';
                            } else {
            ...
            if ( function_exists('bp_is_active') ) {
                ...
                if( is_singular( 'course' )) {
                    ...
                    if ( is_user_logged_in() && $is_user_taking_course ) {
                        ...
                        $html = '<a class="btn inverse" href="'. $button_link .'">' .__( 'Course Discussion', 'boss-sensei') . '</a>';
