...
<?php
	...
	protected function load_component ( $instance ) {
        ...
        if ( is_singular('course') && $queried_object ) {
            $course_id = $queried_object->ID;
        } elseif ( is_singular( 'lesson' ) && $queried_object ) {
            $lesson_id = $queried_object->ID;
            $course_id = get_post_meta( $lesson_id , '_lesson_course', true);
        } elseif ( is_singular( 'quiz' ) ) {
            $lesson_id = $woothemes_sensei->quiz->data->quiz_lesson;
            $course_id = get_post_meta( $lesson_id , '_lesson_course', true);
        }
        ...
        $course_lessons = boss_sensei_course_lessons( $course_id );
        ...
        $html .= '<section class="course-lessons-widgets">';
            ...
            $html .= '<header>';
              $html .= '<h3><a href="'.get_permalink($course_id).'">'.get_the_title($course_id).'</a></h3>';
        ...
        foreach ($modules as $module) {
            ...
            $has_lessons    = count($lessons) > 0 ? true : false;
            $list_arrow     = $has_lessons ? '<div class="list_arrow flippable"><i class="fa fa-sort-desc"></i></div>' : '';

            // module title link
            $html .= '<header><h2>'. $list_arrow .'<span>' . $module->name . '<span class="status module-status ' . esc_attr( $class ) . '">' . $status . '</span></span></h2></header>';
            ...
            if ( $has_lessons ) {
                ...
                foreach ($lessons as $lesson) {
                    ...
                    $lessons_list .= '<li class="' . $status . (( $lesson_id == $lesson->ID )?' current ':'') . '"><a href="' . esc_url(get_permalink(intval($lesson->ID))) . '" title="' . esc_attr(get_the_title(intval($lesson->ID))) . '">' . apply_filters('sensei_module_lesson_list_title', $title, $lesson->ID) . '</a></li>';
                ...
                $html .= '<section class="module-lessons">
                            <header>
                                <h3>'. '<span>' . __('Lessons', 'boss-sensei') . '</span>' . '</h3>
                            </header>
                    <ul>
                        '. $lessons_list . '
                    </ul>
        ...
        if ( 0 < $total_lessons ) {
            $list_arrow     = '<div class="list_arrow flippable"><i class="fa fa-sort-desc"></i></div>';

            $html .= '<section class="lessons-list">';

            $html .= '<header>';

                $html .= '<h2>' . $list_arrow . '<span>' . __( 'Lessons', 'boss-sensei' ). '</span>' . '</h2>';
...
//                $lessons_completed = 0;
                ...
                foreach ( $course_lessons as $lesson_item ){
                    ...
                    if ( $is_preview && !$is_user_taking_course ) {
                        $preview_label = $woothemes_sensei->frontend->sensei_lesson_preview_title_text( $lesson_item->ID );
                        $preview_label = '<span class="preview-heading">' . $preview_label . '</span>';
                        $post_classes[] = 'lesson-preview';
                    }

                    $html .= '<li class="' . esc_attr( join( ' ', get_post_class( $post_classes, $lesson_item->ID ) ) ) . ( $lesson_id == $lesson_item->ID ? ' current' :'' ). '">';
            ...
            if ( is_user_logged_in() && $is_user_taking_course ) {
                ...
                $html .= '<footer>';
                    ...
                    $html .= $author_avatar;
                    $html .= '<p><span>'.__( 'Course by', 'boss-sensei' ).'</p><p>'.$author.'</span></p>';
                    if ( is_user_logged_in() && $is_user_taking_course ) {
                        $button = '';
                        ...
                        if ( !(empty($group_attached) || $group_attached == '-1') )	{
                            ...
                            $button_link = apply_filters( 'boss_edu_group_discussion_button_link', trailingslashit(home_url()).trailingslashit($bp->groups->slug).$group_data );
                            $button = '<p class="bp-group-discussion"><a class="btn inverse" href="'. $button_link .'">'.__('Course Discussion', 'boss-sensei').'</a></p>';
