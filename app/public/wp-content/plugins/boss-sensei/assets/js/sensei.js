...
//    $("#header-menu > ul").fn.jRMenuMore = null;
    ...
    $('#sensei-quiz-list').find('.file-upload').find('input[type="file"]').each(function(){
        $this = $(this);
        $this.wrap("<div class='fake-wrap'></div>")
            .before('<a href="#" class="fake-upload">Choose File</a>')
            .before('<span class="fake-link">No file chosen</span>')
            .change(function(){
    ...
    const $lesson_list_ul = $('.widget_course_progress section.entry, .widget_course_progress .lessons-list .other-lessons').has( ".current" );
    const $module_lesson_ul = $('.widget_course_progress section.entry, .widget_course_progress .module-lessons > ul').has( ".current" );
    $lesson_list_ul.addClass('open').prev().find('.list_arrow').addClass('expand');
    $module_lesson_ul.addClass('open').parents('.post.module').find('.list_arrow').addClass('expand');
    ...
    $('.widget_course_progress .module header, .widget_course_progress .lessons-list header').click(function(e){
        const   $that       = $(this),
                $list_arrow = $that.children().children();

        if ( $that.next().children().length !== 0 ) { // Do not toggle when module dose not contain a lessons
            $that.next().slideToggle();
            $list_arrow.toggleClass('expand collapse');
        }
    ...
    jQuery(document).ready( function($) {
        ...
        $(function prepareList() {
            ...
            $('.expandList')
                .unbind('click')
                .click(function () {
                    $('.collapsed').parents('.module').find('.tog-mod').addClass('fa-chevron-up').removeClass('fa-chevron-down');
            ...
            $('.collapseList')
                .unbind('click')
                .click(function () {
                    $('.expanded').parents('.module').find('.tog-mod').addClass('fa-chevron-down').removeClass('fa-chevron-up');
