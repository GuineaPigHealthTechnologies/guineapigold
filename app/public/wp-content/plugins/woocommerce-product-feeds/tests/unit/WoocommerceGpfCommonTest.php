...
class WoocommerceGpfCommonTest extends WoocommerceGpfTestAbstract {
	...
	public function testFiltersFields() {
		// FIXME
	...
	public function testGetPrepopulateOptions() {
		...
		$wpdb->shouldReceive( 'get_col' )
			->once()
			->with( "SELECT DISTINCT( {$table_prefix}postmeta.meta_key ) FROM {$table_prefix}posts LEFT JOIN {$table_prefix}postmeta ON {$table_prefix}posts.ID = {$table_prefix}postmeta.post_id WHERE {$table_prefix}posts.post_type = 'product'" )
