 ...
 */
define( 'WOOCOMMERCESEA_PLUGIN_VERSION', '2.1.7' );
...
function woosea_request_review(){
	...
	if(($nr_projects > 0) AND ($is_active > $show_after) AND ($notification_interaction != "yes")){
		echo '<div class="notice notice-info review-notification is-dismissible"><font color="green" style="font-weight:bold";><p>Hey, I noticed you have been using my plugin, WooCommerce Product Feed PRO, for over a week now and have created product feed projects with it - that\'s awesome! Could you please do me a BIG favor and give it a 5-star rating on WordPress? Just to help us spread the word and boost my motivation.<br/>~ Joris Verwater<br><ul><li><span class="ui-icon ui-icon-caret-1-e"></span><a href="https://wordpress.org/support/plugin/woo-product-feed-pro/reviews?rate=5#new-post" target="_blank" class="dismiss-review-notification">Ok, you deserve it</a></li><li><span class="ui-icon ui-icon-caret-1-e"></span><a href="#" class="dismiss-review-notification">Nope, maybe later</a></li><li><span class="ui-icon ui-icon-caret-1-e"></span><a href="#" class="dismiss-review-notification">I already did</a></li></ul></p></font></div>';	
	}
}
	...
	if($structured_data_fix == "yes"){
		if ( '' !== $product->get_price() ) {
			...
			if ( $product->is_type( 'variable' ) ) {
				...
				$nr_get = count($_GET);
				if($nr_get > 0){
					// Loop through product variations to find the current one
					foreach ($variations as $var_key => $var_value){

						foreach ($var_value['attributes'] as $attr_key => $attr_value){
							if(isset($_GET[$attr_key]) && ($_GET[$attr_key] == $attr_value)){
								$variable_product = wc_get_product($var_value['variation_id']);
