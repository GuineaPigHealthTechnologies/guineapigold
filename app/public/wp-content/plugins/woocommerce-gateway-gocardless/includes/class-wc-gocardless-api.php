...
class WC_GoCardless_API {
	...
	public static function is_country_supported( $country = '' ) {
		return in_array( $country, self::$supported_countries );
	}
	...
	protected static function _request( $endpoint, $args = array() ) {
		...
		if ( ! empty( $args['body'] ) ) {
			wc_gocardless()->log( sprintf( '%s - %s %s with params: \'%s\'', __METHOD__, $args['method'], $url, print_r( $args['body'], true ) ) );
		} else {
		...
		if ( is_null( $parsed_resp ) ) {
			wc_gocardless()->log( sprintf( '%s - Failed to decode JSON resp %s', __METHOD__, print_r( wp_remote_retrieve_body( $resp ), true ) ) );
			return new WP_Error( 'error_json_decode', __( 'Error decoding JSON response', 'woocommerce-gateway-gocardless' ) );
		}

		if ( ! empty( $parsed_resp['error']['code'] ) && ! empty( $parsed_resp['error']['message'] ) ) {
			wc_gocardless()->log( sprintf( '%s - GoCardless responded with error: %s', __METHOD__, print_r( $parsed_resp['error'], true ) ) );

			$message = $parsed_resp['error']['message'];

			$new_mandate = null;
			if ( ! empty( $parsed_resp['error']['errors'] ) && is_array( $parsed_resp['error']['errors'] ) ) {
				$message .= '. ' . __( 'Error details: ', 'woocommerce-gateway-gocardless' );
				$errors   = array();
	...
	public static function create_redirect_flow( $params = array() ) {
		$args = array(
			'method' => 'POST',
			'body'   => json_encode( array( 'redirect_flows' => $params ) ),
		);
	...
	public static function complete_redirect_flow( $redirect_flow_id, $params = array() ) {
		$args = array(
			'method' => 'POST',
			'body'   => json_encode( array( 'data' => $params ) ),
		);
	...
	public static function update_mandate( $params = array() ) {
		$args = array(
			'method' => 'PUT',
			'body'   => json_encode( array( 'mandates' => $params ) ),
		);
	...
	public static function create_payment( $params = array() ) {
		$args = array(
			'method' => 'POST',
			'body'   => json_encode( array( 'payments' => $params ) ),
		);
	...
	public static function create_subscription( $params = array() ) {
		$args = array(
			'method' => 'POST',
			'body'   => json_encode( array( 'subscriptions' => $params ) ),
		);
	...
	public static function update_customer( $customer_id, $params = array() ) {
		$args = array(
			'method' => 'PUT',
			'body'   => json_encode( array( 'customers' => $params ) ),
		);
	...
	public static function create_refund( $params = array() ) {
		$args = array(
			'method' => 'POST',
			'body'   => json_encode( array( 'refunds' => $params ) ),
		);
